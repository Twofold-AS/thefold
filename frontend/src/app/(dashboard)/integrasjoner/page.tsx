...
import { listIntegrations, saveIntegration, type IntegrationConfig } from "@/lib/api";
...
function isConnected(platform: string, configs: IntegrationConfig[]): boolean {
  ...
  // Static integrations that are always connected by nature
  if (["linear", "github", "resend", "brave-search"].includes(platform)) return true;
  return false;
}
  ...
  const handleConnect = async (platform: string) => {
    try {
      if (platform === "slack" || platform === "discord") {
        await saveIntegration({ platform, enabled: true });
        refresh();
      } else {
        alert(`Tilkobling for ${platform} m\u00e5 konfigureres manuelt.`);
      }
    } catch (e) {
      alert(e instanceof Error ? e.message : "Tilkobling feilet");
    }
  };
  ...
  return (
    <>
      <div style={{ paddingTop: 40, paddingBottom: 24 }}>
        <h2 style={{ fontSize: 28, fontWeight: 600, color: T.text, letterSpacing: "-0.03em", fontFamily: T.brandFont, marginBottom: 8 }}>Integrasjoner</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: `1px solid ${T.border}`, borderTop: "none", borderRadius: `0 0 ${T.r}px ${T.r}px`, position: "relative", overflow: "hidden" }}>
          ...
          {loading ? (
            <div style={{ padding: 40, textAlign: "center", gridColumn: "1 / -1" }}>
              <span style={{ fontSize: 13, color: T.textMuted }}>Laster integrasjoner...</span>
            </div>
          ) : (
            merged.map((ig, i) => {
              const ir = i % 2 === 1;
              ...
              return (
                <div key={i} style={{ padding: 20, borderRight: ir ? "none" : `1px solid ${T.border}`, borderBottom: nl ? `1px solid ${T.border}` : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                    <span style={{ fontSize: 15, fontWeight: 600, color: T.text, fontFamily: T.brandFont }}>{ig.n}</span>
                  ...
                  {!ig.connected && <Btn sm primary onClick={() => handleConnect(ig.platform)}>Koble til</Btn>}
                </div>
              );
            })
          )}
