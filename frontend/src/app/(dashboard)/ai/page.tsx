...
// Model role assignments — based on tag-based selection in the backend
const MODEL_ASSIGNMENTS = [
  { role: "Planner", tag: "planning", fallbackModel: "claude-sonnet-4-5", fallbackProvider: "Anthropic" },
  { role: "Code (complex)", tag: "coding", fallbackModel: "claude-sonnet-4-5", fallbackProvider: "Anthropic" },
  { role: "Code (simple)", tag: "budget", fallbackModel: "deepseek-v3.2", fallbackProvider: "Fireworks" },
  { role: "Tests", tag: "testing", fallbackModel: "qwen3-coder", fallbackProvider: "OpenRouter" },
  { role: "Docs", tag: "chat", fallbackModel: "claude-haiku-4-5", fallbackProvider: "Anthropic" },
  { role: "Research", tag: "analysis", fallbackModel: "gemini-2.5-flash", fallbackProvider: "OpenRouter" },
];
...
export default function AIPage() {
  ...
  // Build role-to-model mapping from actual provider data
  function getModelForRole(tag: string, fallbackModel: string, fallbackProvider: string) {
    for (const provider of providers) {
      for (const model of provider.models || []) {
        if (model.enabled && model.tags.includes(tag)) {
          return { model: model.displayName, provider: provider.name };
        }
      }
    }
                ...
                {!loading && providers.length > 0 && (
                  <div>
                    ...
                    <p className="text-xs mb-4" style={{ color: "var(--tf-text-muted)" }}>
                      Which model handles each role (based on tags)
                    </p>
                    <div
                    ...
                    >
                      <div className="flex items-center gap-3 px-4 py-2" style={{ background: "var(--tf-surface)" }}>
                        <span className="text-[10px] uppercase tracking-wider w-32" style={{ color: "var(--tf-text-faint)" }}>Role</span>
                        <span className="text-[10px] uppercase tracking-wider flex-1" style={{ color: "var(--tf-text-faint)" }}>Model</span>
                        <span className="text-[10px] uppercase tracking-wider w-28 text-right" style={{ color: "var(--tf-text-faint)" }}>Provider</span>
                      </div>
                      {MODEL_ASSIGNMENTS.map((assignment) => {
                        const resolved = getModelForRole(assignment.tag, assignment.fallbackModel, assignment.fallbackProvider);
                        return (
                          <div key={assignment.role} className="flex items-center gap-3 px-4 py-2.5">
                            <span className="text-xs w-32" style={{ color: "var(--tf-text-secondary)" }}>
                              {assignment.role}
                            </span>
                            <span className="text-xs font-mono flex-1 truncate" style={{ color: "var(--tf-text-primary)" }}>
                              {resolved.model}
                            </span>
                            <span className="text-xs w-28 text-right" style={{ color: "var(--tf-text-faint)" }}>
                              {resolved.provider}
                            </span>
                          </div>
                        );
                        ...
                        return (
                          <div key={i} className="flex-1 flex flex-col items-center gap-1">
                            ...
                            <div
                              ...
                              style={{
                                height: `${height}%`,
                                background: "linear-gradient(to top, var(--tf-heat), rgba(122, 170, 206, 0.3))",
                                minHeight: "4px",
                              }}
                            />
                            <span className="text-[10px]" style={{ color: "var(--tf-text-faint)" }}>
