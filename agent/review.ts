...
export async function submitReviewInternal(params: SubmitReviewParams): Promise<{ reviewId: string }> {
  ...
  const row = await db.queryRow<{ id: string }>`
    INSERT INTO code_reviews (
      conversation_id, task_id, task_description, project_task_id, sandbox_id,
...
export const getReview = api(
  ...
  async (req: GetReviewRequest): Promise<{ review: CodeReview }> => {
    const row = await db.queryRow<CodeReviewRow>`
      SELECT id, conversation_id, task_id, task_description, project_task_id, sandbox_id,
...
export const approveReview = api(
  ...
  async (req: ApproveReviewRequest): Promise<{ prUrl: string }> => {
    ...
    const taskDescription = review.taskDescription || review.taskId;
    const branchName = generateBranchName(taskDescription, review.taskId);
    const commitMessage = generateConventionalCommit(taskDescription, review.filesChanged);
    const prBody = buildPRDescription(taskDescription, review.filesChanged, review.aiReview?.documentation || "", []);
    ...
    try {
      pr = await github.createPR({
        ...
        repo: targetRepo,
        branch: branchName,
        title: `[TheFold] ${taskDescription.split("\n")[0].substring(0, 72)}`,
        body: prBody,
        commitMessage,
