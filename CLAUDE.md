...
- `agent/context-builder.ts` — STEP 2+3+3.5: buildContext(ctx, tracker, helpers) → AgentContext. GitHub tree, relevant files, memory search, docs lookup, MCP tools. STEP 2.5 (YD): Import-graf integrasjon via buildImportGraph, getRelatedFiles (depth 2), hent maks 5 ekstra avhengigheter. YA: filterForPhase(context, phase) filters context per-phase (confidence/planning/building/diagnosis/reviewing/completing) with token budgets (3K-50K). estimateTokens() estimates tokens (1 token ≈ 4 chars). trimContext() trims least important fields first (docs → memory → files → tree). ~30% token reduction.
- `agent/code-graph.ts` — YD: Import-graf analyse for presis filvalg. extractImports() regex-parser (ES6/CommonJS), resolveImport() path resolution med extensions, buildImportGraph() bidireksjonal graf (imports + importedBy), getRelatedFiles() traverserer begge retninger med maxDepth. Kopierer logikk fra builder/graph.ts (cross-service regel). ~15-25% presisjon-forbedring i filvalg.
...
- `agent/execution.ts` — STEP 5-7+retry: executePlan(ctx, contextData, tracker, helpers, options) → ExecutionResult. Plan, error patterns, sub-agents, build, validate, retry loop with 6 diagnosis branches. YB: computeRetryContext() beregner delta (taskSummary 200 chars, changedFiles diff, latestError 1000 chars). computeSimpleDiff() linje-basert diff (maks 20 linjer/500 chars). implementation_error + default branches bruker delta-context → ~60-75% token-sparing per retry.
